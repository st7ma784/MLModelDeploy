...
The idea is that requests can be unpredictable if directly querying a model. Instead it is better to push requests onto a queue and allow the model to pop them.
...
This is what we set our nomad command to alongside a redis stream instance locally - see the nomad script. 
